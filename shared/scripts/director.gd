...
func _show_game_over_screen() -> void:
	...
	# Position buttons below score (find the vbox container)
...
func _on_play_again_pressed() -> void:
	# Remove the play again and share buttons
	for child in ui_layer.get_children():
		if child is CenterContainer:
			for subchild in child.get_children():
				if subchild is VBoxContainer:
					var play_button = subchild.get_node_or_null("PlayAgainButton")
					if play_button:
						play_button.queue_free()
					var share_button = subchild.get_node_or_null("ShareButton")
					if share_button:
						share_button.queue_free()
					break
			break
